...
- `ScalarConverter.cpp` — all helpers live in an anonymous namespace (they
  ...
  - `isCharLiteral` / `isPseudoLiteral` / `isNumericLiteral` classify the
    input (decimal notation only, per subject). `isNumericLiteral` recognises
    int, double and float literals in a single pass over the grammar
    `[+-]? (digit | '.')* 'f'?` — one dot at most, one digit at least —
    replacing three near-identical scanners.
  - Parsing funnels every non-char literal through one `std::strtod` call into
    a `double` (`double` represents every `char`, `int`, and `float` value
    exactly enough for this exercise). That includes the pseudo-literals:
    `strtod` parses `nan`/`inf`/`-inf` natively and stops before a trailing
    `f`, so `nanf` and `+inff` need no special case. Using `strtod` instead of
    `atoi` means an out-of-range integer literal like `9999999999` becomes a
    large finite double and is reported as `int: impossible` instead of
