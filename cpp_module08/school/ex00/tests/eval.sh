...
#
#   ./tests/eval.sh      (from ex00/ or from tests/)
#
# Sheet items covered:
...
#   ex00           easyFind(T, int) is a template and HAS to use STL algorithms
#                  (a manual iterator search counts as wrong)
...
if $CXX $STD -I. "$OUT/tu1.cpp" "$OUT/tu2.cpp" -o "$OUT/two" \
  ...
  pass "no-function-in-header" \
    "two TUs including the header link: only templates inside"
else
  fail "no-function-in-header" \
    "a non-template function is defined in a header" "$(cat "$OUT/odr.log")"
fi

# A header that only compiles because something else was included first is a
# trap for whoever includes it next.
if $CXX $STD $WARN -I. -fsyntax-only "$OUT/tu2.cpp" 2>"$OUT/self.log"; then
  pass "header-self-contained" "the header compiles on its own"
else
...
# one must be a file that ships with the exercise. Anything else is an
# external library, which the sheet flags.
...
# C++11-and-later spellings. -pedantic-errors above already rejects most of
# them; this names the one that slipped through instead of dumping a compiler
# diagnostic on the reader.
...
absent "no-cxx11-library" \
  "std::(unordered_[a-z]+|array|to_string|move|forward|shared_ptr|unique_ptr|nullptr_t|begin|end)\b" \
...
# LIBS/LDFLAGS empty means nothing external gets linked in.
...
info "no-class-in-turn-in" "$EX ships a function template, not a class"
...
# ── ex00-specific sheet items ────────────────────────────────────────────────
heading "$EX specific"

present "is-a-template" "^template <typename T>" "$HEADERS" \
  "easyfind is a function template over T"
present "takes-t-and-int" "easyfind\((const )?T& container, int value\)" "$HEADERS" \
  "signature is (T, int) as the subject spells it"
present "uses-stl-algorithm" "std::find *\(" "$HEADERS" \
  "the search IS an STL algorithm - std::find"
present "has-const-overload" "typename T::const_iterator easyfind" "$HEADERS" \
  "a const overload, so const containers are searchable"

# The sheet's exact words: "It HAS to use STL algorithms. If it does not (like
# manual search using iterators for example), count it as wrong." A correct
# hand-written loop passes every runtime test ever written, so grep is the
# only instrument that can see this one.
loops=$(grep -nE "(^|[^_a-zA-Z])(for|while) *\(" $HEADERS)
if [ -z "$loops" ]; then
  pass "no-manual-search" "not one loop in the header: the algorithm does it"
else
  fail "no-manual-search" "a hand-written search loop is graded as wrong" \
    "$loops"
fi

# std::find is not enough on its own if the result is then walked by hand.
walks=$(grep -nE "(\+\+|--) *it *;" $HEADERS)
if [ -z "$walks" ]; then
  pass "no-manual-walk" "the iterator is returned, never advanced by hand"
else
  fail "no-manual-walk" "the header advances the iterator itself" "$walks"
fi
