...
# test.cpp also drives bigint and vect2, so they have to travel with it.
...
cp bigint.hpp bigint.cpp vect2.hpp vect2.cpp "$OUT/"
...
if ! $CXX $STD -w "$OUT/tests/test.cpp" $AUX -o "$OUT/baseline" 2>"$OUT/baseline.log"; then
  printf "${RED}${BOLD}baseline does not compile${OFF} - the harness is broken, not iter.hpp\n"
