...
# The eval sheet's "reading only if the instance is const", spelled out. The
# const overload returns const T&, so the write is refused on a class element
# exactly as it is on an int - and the element's own const members still work.
expect_ok "const-array-reads" "a const Array<bigint> reads fine" <<'EOF'
#include "Array.hpp"
#include "bigint.hpp"
void f(const Array<bigint>& a) { (void)a[0].digits(); }
EOF

expect_fail "const-array-write-complex" "const T& refuses the write on a class too" <<'EOF'
#include "Array.hpp"
#include "bigint.hpp"
void f(const Array<bigint>& a) { a[0] = bigint(1); }
EOF

expect_fail "const-array-nonconst-member" "and refuses a mutating member call" <<'EOF'
#include "Array.hpp"
#include "vect2.hpp"
void f(const Array<vect2>& a) { ++a[0]; }
EOF

# The iter bridge. &a[0] on a const Array is a const T*, so iter deduces
# T = const T and a mutating callback cannot bind - the const-ness travels
# from the Array, through the pointer, into the template, out to the callback.
expect_ok "iter-bridge" "iter can walk an Array through &a[0]" <<'EOF'
#include "Array.hpp"
#include "iter.hpp"
#include "vect2.hpp"
void bump(vect2& v) { ++v; }
void f() {
  Array<vect2> a(3);
  if (a.size() != 0) ::iter(&a[0], a.size(), bump);
}
EOF

expect_fail "iter-bridge-const" "const rides from the Array into iter" <<'EOF'
#include "Array.hpp"
#include "iter.hpp"
#include "vect2.hpp"
void bump(vect2& v) { ++v; }
void f(const Array<vect2>& a) {
  if (a.size() != 0) ::iter(&a[0], a.size(), bump);  // &a[0] is const vect2*
}
EOF
