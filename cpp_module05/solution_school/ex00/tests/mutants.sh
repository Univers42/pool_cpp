#!/usr/bin/env bash
# =============================================================================
#  tests/mutants.sh — a mutation-testing drill for ex00
# =============================================================================
#
...
printf "\n%s%s Sabotage drill — ex00%s\n" "$BOLD" "$CYAN" "$RESET"
printf "%s Each line breaks the code on purpose; DETECTED is the good outcome.%s\n\n" "$DIM" "$RESET"

# ── The bounds themselves ───────────────────────────────────────────────────
#   Widens the upper bound so grade 0 is accepted. A bureaucrat could exist
#   outside [1,150].
mutate "validateGrade lets grade 0 through" Bureaucrat.cpp \
  's/if (grade < 1)/if (grade < 0)/'
#   Widens the lower bound so grade 151 is accepted.
mutate "validateGrade lets grade 151 through" Bureaucrat.cpp \
  's/if (grade > 150)/if (grade > 151)/'
#   Narrows the bound so the legal best grade, 1, is refused.
mutate "validateGrade rejects the legal grade 1" Bureaucrat.cpp \
  's/if (grade < 1)/if (grade < 2)/'
#   Narrows the other bound so the legal worst grade, 150, is refused.
mutate "validateGrade rejects the legal grade 150" Bureaucrat.cpp \
  's/if (grade > 150)/if (grade > 149)/'
#   Throws TooLow where TooHigh is meant and vice versa. Both bounds still
#   throw, so only a test that names the type notices.
mutate "the two bound checks are swapped" Bureaucrat.cpp \
...
# ── Direction: 1 is the highest, so increment SUBTRACTS ─────────────────────
...
#   Makes decrement SUBTRACT instead of add — the mirror of the bug above.
mutate "decrementGrade goes the wrong way" Bureaucrat.cpp \
  's/void Bureaucrat::decrementGrade() { _grade = validateGrade(_grade + 1); }/void Bureaucrat::decrementGrade() { _grade = validateGrade(_grade - 1); }/'
#   Right direction, wrong distance: jumps two grades per call instead of one.
mutate "incrementGrade moves by two" Bureaucrat.cpp \
  's/void Bureaucrat::incrementGrade() { _grade = validateGrade(_grade - 1); }/void Bureaucrat::incrementGrade() { _grade = validateGrade(_grade - 2); }/'

# ── An operator with its own, divergent bounds check ────────────────────────
# This is what the every-form-every-grade sweep exists to catch: the object
# still behaves correctly at the values a hand-written test would pick.
...
#   Same for --, which can then walk past 150.
mutate "prefix -- bypasses validateGrade" Bureaucrat.cpp \
  's/Bureaucrat& Bureaucrat::operator--() { return (decrementGrade(), \*this); }/Bureaucrat\& Bureaucrat::operator--() { ++_grade; return (*this); }/'
#   ++ has its own check that stops at 2 rather than 1, so the top grade is
#   unreachable.
mutate "prefix ++ guards one short of the bound" Bureaucrat.cpp \
  's/Bureaucrat& Bureaucrat::operator++() { return (incrementGrade(), \*this); }/Bureaucrat\& Bureaucrat::operator++() { if (_grade > 2) --_grade; return (*this); }/'

# These two are the reason the every-form-every-grade sweep exists: both behave
# perfectly at 1, 3, 42, 150 — every value a hand-written test would pick — and
# are wrong only deep inside the range. Only an exhaustive sweep sees them.
#   Correct everywhere EXCEPT grade 100. Passes any hand-picked value; only an
#   every-grade sweep sees it.
mutate "prefix ++ is a no-op at grade 100 only" Bureaucrat.cpp \
  's/Bureaucrat& Bureaucrat::operator++() { return (incrementGrade(), \*this); }/Bureaucrat\& Bureaucrat::operator++() { if (_grade == 100) return (*this); return (incrementGrade(), *this); }/'
#   Correct postfix semantics everywhere except grade 77. Again, only an
#   exhaustive sweep catches it.
mutate "postfix -- returns the new value at grade 77 only" Bureaucrat.cpp \
  's/Bureaucrat Bureaucrat::operator--(int) { Bureaucrat tmp(\*this); decrementGrade(); return (tmp); }/Bureaucrat Bureaucrat::operator--(int) { if (_grade == 77) { decrementGrade(); return (*this); } Bureaucrat tmp(*this); decrementGrade(); return (tmp); }/'

# A throw site that is not a std::exception at all. The compile-time upcast in
# test_exception_hierarchy still passes here — only the per-site runtime checks
# notice, which is why both halves of that section are needed.
#   Throws `42` instead of an exception type. The classes still derive from
#   std::exception, so only the per-site runtime checks notice.
mutate "incrementGrade throws a bare int at the ceiling" Bureaucrat.cpp \
  's/void Bureaucrat::incrementGrade() { _grade = validateGrade(_grade - 1); }/void Bureaucrat::incrementGrade() { if (_grade <= 1) throw 42; _grade = validateGrade(_grade - 1); }/'

# ── Postfix must hand back the value from BEFORE ────────────────────────────
...
#   The same mistake on the downward postfix operator.
mutate "postfix -- returns the new value" Bureaucrat.cpp \
  's/Bureaucrat Bureaucrat::operator--(int) { Bureaucrat tmp(\*this); decrementGrade(); return (tmp); }/Bureaucrat Bureaucrat::operator--(int) { decrementGrade(); Bureaucrat tmp(*this); return (tmp); }/'
#   Returns the right snapshot but never changes the object.
mutate "postfix ++ forgets to mutate at all" Bureaucrat.cpp \
  's/Bureaucrat Bureaucrat::operator++(int) { Bureaucrat tmp(\*this); incrementGrade(); return (tmp); }/Bureaucrat Bureaucrat::operator++(int) { Bureaucrat tmp(*this); return (tmp); }/'

# ── The exception hierarchy ─────────────────────────────────────────────────
# Dropping the base makes the class still look like an exception and still be
# catchable by its own name — only a std::exception& handler notices.
#   Removes `: public std::exception`. The class still looks like an exception
#   and is still catchable by its own name — only a std::exception& handler
#   notices.
mutate "GradeTooHigh stops deriving from std::exception" Bureaucrat.hpp \
  's/class GradeTooHighException : public std::exception {/class GradeTooHighException {/'
#   Same, on the other exception type.
mutate "GradeTooLow stops deriving from std::exception" Bureaucrat.hpp \
  's/class GradeTooLowException : public std::exception {/class GradeTooLowException {/'
#   Makes both what() strings identical, so a caught exception cannot tell you
#   which bound was hit.
mutate "both bounds report the same message" Bureaucrat.cpp \
  's/return ("Grade too low (lowest is 150)")/return ("Grade too high (highest is 1)")/'
#   Keeps the type but empties the message, leaving nothing to print in a
#   diagnostic.
mutate "what() returns an empty string" Bureaucrat.cpp \
  's/return ("Grade too high (highest is 1)")/return ("")/'

# ── operator<< and the OCF basics ───────────────────────────────────────────
#   Removes the '.' the subject's output format requires.
mutate "operator<< drops the trailing period" Bureaucrat.cpp \
  's/ << "\."//'
#   Swaps the two fields, so both values still appear but in the wrong order.
mutate "operator<< prints grade before name" Bureaucrat.cpp \
  's/b.getName() << ", bureaucrat grade " << b.getGrade()/b.getGrade() << ", bureaucrat grade " << b.getName()/'
