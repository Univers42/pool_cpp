...
// The forward declaration in the header is enough to *declare* signForm, but
// calling beSigned() and getName() needs the complete type.
#include "Form.hpp"
...
// SIGNING
// beSigned() is the one that decides and throws; this only reports. Catching
// by std::exception& (not by Form::GradeTooLowException&) keeps the reason
// generic, so a new refusal reason in Form needs no change here.
