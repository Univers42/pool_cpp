#!/usr/bin/env bash
# =============================================================================
#  tests/mutants.sh — a mutation-testing drill for ex03
# =============================================================================
#
...
SRCS="AForm.cpp Bureaucrat.cpp Intern.cpp ShrubberyCreationForm.cpp RobotomyRequestForm.cpp PresidentialPardonForm.cpp"
...
printf "\n%s%s Sabotage drill — ex03%s\n" "$BOLD" "$CYAN" "$RESET"
printf "%s Each line breaks the code on purpose; DETECTED is the good outcome.%s\n\n" "$DIM" "$RESET"

# ── The execute() gate — inherited from ex02, still load-bearing here ───────
...
# ── The Intern: the name -> type mapping, which is all of ex03 ──────────────
# Swapping the two KEYS rather than a creator keeps every creator referenced,
# so this fails in the suite instead of being rejected as an unused function.
#   Swaps the KEYS of two table rows, so 'shrubbery creation' builds a
#   robotomy and vice versa. Every creator stays referenced, so this compiles
#   and the SUITE has to catch it.
mutate "table swaps two name -> type rows" Intern.cpp \
  's/{"shrubbery creation", \&makeShrubbery}/{"robotomy request", \&makeShrubbery}/; s/{"robotomy request", \&makeRobotomy}/{"shrubbery creation", \&makeRobotomy}/'
#   Misspells one key, making that form unreachable by its documented name.
mutate "a table key has a typo" Intern.cpp \
  's/{"robotomy request",/{"robotomy requst",/'
#   Removes the name comparison, so every request returns a
#   ShrubberyCreationForm.
mutate "makeForm ignores the name and always returns the first" Intern.cpp \
  's/    if (formName != kForms\[i\].name) continue;//'
# `+ target.substr(0, 0)` keeps `target` referenced so the mutant compiles and
# the wrong value has to be caught by the suite, not by -Wunused-parameter.
#   Builds the right type with the wrong target. `+ target.substr(0,0)` keeps
#   `target` referenced so this still compiles.
mutate "makeForm passes the name as the target" Intern.cpp \
  's/kForms\[i\].create(target)/kForms[i].create(formName + target.substr(0, 0))/'
#   Drops the 'Intern creates <form>' line the subject requires.
mutate "makeForm never announces the creation" Intern.cpp \
  's|std::cout << "Intern creates " << form->getName() << std::endl;||'
#   Builds the form then throws the pointer away, leaking it and returning
#   NULL.
mutate "makeForm returns NULL for a known name" Intern.cpp \
  's/^    return (form);/    return (NULL);/'
#   Returns a fallback form for an unrecognised name instead of failing.
mutate "unknown name yields a form instead of NULL" Intern.cpp \
  's/^  return (NULL);/  return (makeShrubbery(target));/'
#   Returns NULL correctly but prints no error message.
mutate "unknown name fails silently" Intern.cpp \
  's|std::cout << "Intern cannot create |if (false) std::cout << "Intern cannot create |'
