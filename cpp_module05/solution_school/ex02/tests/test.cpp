// Standalone C++98 test suite for ex02: the abstract AForm, its three concrete
// forms, and the execute() gate.
//
// Covers: everything ex01 checked for Form (bounds, exception identity, the
// beSigned rank rule, OCF under const members) plus what ex02 adds — the fixed
...
// effect, and the two sentences executeForm() must print.
//
...
int main() {
  std::cout << "AForm test suite" << std::endl;
