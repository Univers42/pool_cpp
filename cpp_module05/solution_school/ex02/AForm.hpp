...
class AForm {
 public:
  ...
  // execute() needs a third failure mode that ex01's Form had no use for.
  ...
  // VIRTUAL, and this is not optional: `delete` through an AForm* must run the
  // concrete form's destructor first. Without it, ~ShrubberyCreationForm never
  // runs and its _target string leaks.
  virtual ~AForm();
  ...
  // The gate. Throws NotSignedException if the form is unsigned, or
  // GradeTooLowException if `executor` does not reach _gradeToExecute; only
  // then does it call executeAction(). Deliberately non-virtual.
  void execute(const Bureaucrat& executor) const;

 protected:
  // The hook. Pure virtual, which is what makes AForm abstract — no `AForm f;`
  // will compile. `const` because executing a form must not mutate it.
  // Protected, so only the class hierarchy can invoke it: outside code must go
  // through execute() and therefore through the checks.
