...
class Bureaucrat {
 public:
  ...
  // The two members that talk to a form. Both swallow the exception and print
  // the outcome instead, because the subject asks for a printed reason rather
  // than a throw — so callers never need a try/catch around either.
  //
  // signForm takes a non-const reference (signing mutates the form);
  // executeForm takes a const one and is itself const (executing changes
  // neither the form nor the bureaucrat).
